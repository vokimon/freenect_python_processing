...
if __name__ == "__main__" :
	...
	size = 480,360
	rate = 40
	video = VideoSink(size, "test", rate=rate, byteorder="bgra")
	image = np.ones(size, np.uint32)
	for color in (
		0x00ff0000, # red
		0x0000ff00, # green
		0x000000ff, # blue
	...
	for col in xrange(0,size[0],2) :
		image[col,:] = 0x0000ff00
		video.run(image)
	# Turn blue from up to down
	for row in xrange(0,size[1],2) :
		image[:,row] = 0x000000ff
		video.run(image)
	for r in xrange(400) :
		video.run(np.fromfunction(lambda g,b : np.uint32(b)+0x100*g+0x10000*r,  shape=size, dtype=np.uint32))
		print ".",
		sys.stdin.flush()
	video.close()
